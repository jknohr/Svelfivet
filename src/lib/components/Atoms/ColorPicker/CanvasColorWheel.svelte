...
<script lang="ts">
	...
	let { size = 200, parameterStore = null } = $props<{
		size?: number;
		parameterStore: CSSColorString | null;
	}>();

	// State declarations
	let picker: HTMLDivElement;
	let picking = $state(false);
	let wheel = $state(true);
	let wheelTop = $state(0);
	let wheelLeft = $state(0);
	let pickerX = $state(size / 2);
	let pickerY = $state(size / 2);
	
	// Derived values
	let cursorX = $derived(cursor.x);
	let cursorY = $derived(cursor.y);

	// Update picker position when parameterStore changes
	$effect(() => {
		const pos = colorToPickerXY(parameterStore, size);
		pickerX = pos.pickerX;
		pickerY = pos.pickerY;
	});
	...
	$effect(() => {
		if (picking) {
			...
			const pos = colorToPickerXY(pickedColor, size);
			pickerX = pos.pickerX;
			pickerY = pos.pickerY;
	...
	function colorToPickerXY(
		...
		if (!color) return { pickerX: size / 2, pickerY: size / 2 };
