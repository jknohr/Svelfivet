...
<script lang="ts">
	...
	// State declarations
	let edgeWidth = $state<number | undefined>(undefined);
	let color = $state<CSSColorString | undefined>(undefined);
	let straight = $state<boolean | undefined>(undefined); // Stretch feature, requires additional logic
	let step = $state<boolean | undefined>(undefined);
	let cornerRadius = $state<number | undefined>(undefined);
	let animate = $state<boolean | undefined>(undefined);
	let edgeLabel = $state<string | undefined>(undefined);
	let labelColor = $state<CSSColorString | undefined>(undefined);
	let textColor = $state<CSSColorString | undefined>(undefined);
	let targetColor = $state<CSSColorString | undefined>(undefined); // Stretch feature, needs edgeClick to function
