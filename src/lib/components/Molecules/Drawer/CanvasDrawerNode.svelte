...
<script lang="ts">
	import type {
		...
		CSSColorString,
		NodeProps
	} from '$lib/components/Drawer/Drawer.types.ts';
	import { addProps } from '$lib/utils';

	// Replace store with state
	let defaultNodeProps = $state<Array<NodeDrawerConfig>>([]);

	// State declarations
	let bgColor = $state<CSSColorString | undefined>(undefined);
	let borderColor = $state<CSSColorString | undefined>(undefined);
	let label = $state<string | undefined>(undefined);
	let width = $state(200);
	let height = $state(100);
	let locked = $state<boolean | undefined>(undefined);
	let center = $state<boolean | undefined>(undefined);
	let inputs = $state<number | undefined>(undefined);
	let outputs = $state<number | undefined>(undefined);
	let rotation = $state<number | undefined>(undefined);
	let zIndex = $state<number | undefined>(undefined);
	let TD = $state<boolean | undefined>(undefined);
	let LR = $state<boolean | undefined>(undefined);
	let useDefaults = $state<boolean | undefined>(undefined);
	let nodeDirection = $state<string | undefined>(undefined);

	// Props
	let { children } = $props();

	// Creates props and adds to defaultNodeProps
	export const createNodeProps = (
		...
		defaultNodeProps = [...defaultNodeProps, nodeProps];
	};
